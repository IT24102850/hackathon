...
selected_tab = option_menu(
    menu_title=None,
    options=["3D RISK MAP", "ANALYTICS", "DISTRICT TABLE", "ALERTS & INFO", "FORECAST", "VISION AI", "SAFE CENTRES"],
    icons=["map", "bar-chart-line", "table", "shield-exclamation", "calendar3", "camera", "shield-heart"],
    default_index=0,
...
# ============================================================================
# TAB 7: SAFE CENTRES DIRECTORY (FR-3.1 - FR-3.8)
# ============================================================================
if selected_tab == "SAFE CENTRES":
    st.markdown("""
        <div style="margin-bottom: 1.5rem;">
            <span class="gradient-text" style="font-size: 1.5rem; font-weight: 700;">Registered Safe Evacuation Centres</span>
            <p style="color: #8A8F9E; margin-top: 0.25rem;">Real-time safe shelter capacity, occupancy visualizer, and direct-contact hotline links</p>
        </div>
    """, unsafe_allow_html=True)

    CENTRES_DATA = [
        {"name": "Kalutara Vidyalaya National Relief Shelter", "type": "School", "town": "Kalutara North", "district": "Kalutara", "phone": "+94 34 222 2244", "capacity": 450, "occupancy": 380},
        {"name": "Nagoda Community Disaster Centre", "type": "Community Center", "town": "Nagoda", "district": "Kalutara", "phone": "+94 34 223 1188", "capacity": 200, "occupancy": 200},
        {"name": "Ferguson High School Evacuation Shelter", "type": "School", "town": "Ratnapura Central", "district": "Ratnapura", "phone": "+94 45 222 2315", "capacity": 500, "occupancy": 320},
        {"name": "Kuruwita Town Hall Safe Haven", "type": "Municipal Hall", "town": "Kuruwita", "district": "Ratnapura", "phone": "+94 45 226 2110", "capacity": 300, "occupancy": 150},
        {"name": "Nivithigala Pradeshiya Sabha Relief Hall", "type": "Community Center", "town": "Nivithigala", "district": "Ratnapura", "phone": "+94 45 227 9400", "capacity": 250, "occupancy": 250},
        {"name": "Kaduwela Mahaweli Cultural Center", "type": "Community Center", "town": "Kaduwela", "district": "Colombo", "phone": "+94 11 253 6780", "capacity": 350, "occupancy": 290},
        {"name": "Kolonnawa Balika Vidyalaya Shelter", "type": "School", "town": "Kolonnawa", "district": "Colombo", "phone": "+94 11 257 2341", "capacity": 400, "occupancy": 400},
        {"name": "Wellampitiya Youth Vocational Center", "type": "Youth Center", "town": "Wellampitiya", "district": "Colombo", "phone": "+94 11 254 8899", "capacity": 220, "occupancy": 140},
        {"name": "Biyagama Central College Emergency Camp", "type": "School", "town": "Biyagama", "district": "Gampaha", "phone": "+94 11 248 8122", "capacity": 380, "occupancy": 210},
        {"name": "Kelaniya Raja Maha Vihara Pilgrim Rest", "type": "Religious Shelter", "town": "Kelaniya", "district": "Gampaha", "phone": "+94 11 291 1255", "capacity": 600, "occupancy": 410},
        {"name": "Galle Municipal Indoor Stadium Complex", "type": "Stadium", "town": "Galle Fort", "district": "Galle", "phone": "+94 91 223 4567", "capacity": 750, "occupancy": 490},
        {"name": "Baddegama Central College Shelter", "type": "School", "town": "Baddegama", "district": "Galle", "phone": "+94 91 229 2340", "capacity": 320, "occupancy": 320},
        {"name": "St. Thomas' College Flood Evacuation Hall", "type": "School", "town": "Matara Town", "district": "Matara", "phone": "+94 41 222 2580", "capacity": 500, "occupancy": 275},
        {"name": "Thihagoda Agrarian Service Center", "type": "Community Center", "town": "Thihagoda", "district": "Matara", "phone": "+94 41 224 5120", "capacity": 180, "occupancy": 95},
        {"name": "Kegalu Vidyalaya Disaster Relief Base", "type": "School", "town": "Kegalle Town", "district": "Kegalle", "phone": "+94 35 222 2355", "capacity": 420, "occupancy": 290},
        {"name": "Aranayaka Community Landslide Shelter", "type": "Community Center", "town": "Aranayaka", "district": "Kegalle", "phone": "+94 35 225 8100", "capacity": 260, "occupancy": 180},
        {"name": "Kurunegala Town Hall Assembly Auditorium", "type": "Municipal Hall", "town": "Kurunegala City", "district": "Kurunegala", "phone": "+94 37 222 2275", "capacity": 650, "occupancy": 380},
        {"name": "Ibbagamuwa Central College Hall", "type": "School", "town": "Ibbagamuwa", "district": "Kurunegala", "phone": "+94 37 225 9411", "capacity": 350, "occupancy": 350},
        {"name": "Peradeniya Community Recreation Hall", "type": "Community Center", "town": "Peradeniya", "district": "Kandy", "phone": "+94 81 238 8200", "capacity": 300, "occupancy": 120},
        {"name": "Tangalle Municipal Cultural Center", "type": "Community Center", "town": "Tangalle", "district": "Hambantota", "phone": "+94 47 224 0244", "capacity": 280, "occupancy": 110}
    ]

    # Calculate remaining places (FR-3.2)
    for c in CENTRES_DATA:
        c["places_free"] = max(0, c["capacity"] - c["occupancy"])
        c["is_full"] = (c["places_free"] == 0)
        c["pct"] = min(100, int(round((c["occupancy"] / c["capacity"]) * 100))) if c["capacity"] > 0 else 0

    # Controls row
    ctrl_col1, ctrl_col2, ctrl_col3 = st.columns([2, 1.5, 1.5])
    with ctrl_col1:
        search_kw = st.text_input("🔍 Search centres (Name, Town, or Type)", placeholder="e.g. Vidyalaya, Ratnapura, School").strip().lower()

    with ctrl_col2:
        districts_list = sorted(list(set(c["district"] for c in CENTRES_DATA)))
        selected_district = st.selectbox("📍 Filter by District", ["All Districts"] + districts_list)

    with ctrl_col3:
        space_only = st.checkbox("🟢 Only centres with space available", value=False)

    # Filter data (FR-3.4, FR-3.5, FR-3.6 combined)
    filtered_centres = []
    for c in CENTRES_DATA:
        match_search = (not search_kw) or (search_kw in c["name"].lower()) or (search_kw in c["town"].lower()) or (search_kw in c["type"].lower())
        match_district = (selected_district == "All Districts") or (c["district"] == selected_district)
        match_space = (not space_only) or (c["places_free"] > 0)
        if match_search and match_district and match_space:
            filtered_centres.append(c)

    # Availability Summary (FR-3.7)
    total_matching = len(filtered_centres)
    total_free_places = sum(c["places_free"] for c in filtered_centres)
    total_capacity = sum(c["capacity"] for c in filtered_centres)

    sum_c1, sum_c2, sum_c3 = st.columns(3)
    with sum_c1:
        st.metric("Centres Matching", f"{total_matching} of {len(CENTRES_DATA)}")
    with sum_c2:
        st.metric("Total Places Free", f"{total_free_places:,}", delta=f"{total_capacity:,} total cap")
    with sum_c3:
        occupancy_rate = ((total_capacity - total_free_places) / total_capacity * 100) if total_capacity > 0 else 0
        st.metric("Average Fill Rate", f"{occupancy_rate:.1f}%")

    st.markdown("<br>", unsafe_allow_html=True)

    if not filtered_centres:
        st.warning("No centres match your filter criteria. Try resetting search or checking full centres.")
    else:
        # Display centres grid
        card_cols = st.columns(2)
        for idx, centre in enumerate(filtered_centres):
            col_target = card_cols[idx % 2]
            with col_target:
                status_badge = '<span style="background:#ef4444;color:white;padding:3px 8px;border-radius:6px;font-weight:bold;font-size:0.8rem;">FULL</span>' if centre["is_full"] else f'<span style="background:#10b981;color:white;padding:3px 8px;border-radius:6px;font-weight:bold;font-size:0.8rem;">{centre["places_free"]} FREE</span>'
                bar_color = "#ef4444" if centre["pct"] >= 100 else ("#f59e0b" if centre["pct"] >= 80 else "#00c9ff")
                clean_phone = "".join(ch for ch in centre["phone"] if ch.isdigit() or ch == "+")
                
                st.markdown(f"""
                    <div style="background: rgba(20,25,40,0.7); border: 1px solid {'rgba(239,68,68,0.4)' if centre['is_full'] else 'rgba(255,255,255,0.08)'}; border-radius: 14px; padding: 1.25rem; margin-bottom: 1rem;">
                        <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom: 0.5rem;">
                            <h4 style="margin:0; color:#ffffff; font-size:1.05rem;">{centre['name']}</h4>
                            {status_badge}
                        </div>
                        <div style="margin-bottom:0.75rem; font-size:0.82rem; color:#8A8F9E;">
                            <span>🏷️ {centre['type']}</span> | <span>📍 {centre['town']}, <strong>{centre['district']}</strong></span>
                        </div>
                        <div style="margin-bottom:0.5rem;">
                            <div style="display:flex; justify-content:space-between; font-size:0.8rem; margin-bottom:4px;">
                                <span>Occupancy: <strong>{centre['occupancy']} / {centre['capacity']}</strong></span>
                                <span style="font-weight:bold; color:{bar_color};">{centre['pct']}%</span>
                            </div>
                            <div style="width:100%; height:8px; background:rgba(255,255,255,0.08); border-radius:4px; overflow:hidden;">
                                <div style="width:{centre['pct']}%; height:100%; background:{bar_color};"></div>
                            </div>
                        </div>
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-top:0.75rem; font-size:0.85rem;">
                            <span style="color:{'#ef4444' if centre['is_full'] else '#10b981'}; font-weight:600;">
                                {'Centre is Full' if centre['is_full'] else f"{centre['places_free']} places remaining"}
                            </span>
                            <a href="tel:{clean_phone}" style="display:inline-flex; align-items:center; gap:4px; background:rgba(0,201,255,0.15); color:#00c9ff; padding:4px 10px; border-radius:8px; text-decoration:none; font-weight:600; font-size:0.82rem; border:1px solid rgba(0,201,255,0.3);">
                                📞 Call {centre['phone']}
                            </a>
                        </div>
                    </div>
